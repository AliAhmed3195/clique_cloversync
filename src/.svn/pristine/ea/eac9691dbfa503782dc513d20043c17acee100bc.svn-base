...
angular.module('invoice').controller('ColorGeneratorCtrl',
function ($scope) {
	$scope.mColorsHSL = [];
	$scope.baseColor = "#E91E63";
	$scope.$watch('baseColor', function(newValue, oldValue){
		$scope.rgbColor=hexToRgb(newValue);
		if($scope.rgbColor !== null){
			$scope.hsvColor = ColorConverter.toHSV(new RGB($scope.rgbColor.r, $scope.rgbColor.g, $scope.rgbColor.b));
			console.log($scope.rgbColor);
			console.log($scope.hsvColor);
			$scope.mColors = materialColors($scope.rgbColor);
			console.log($scope.mColors);
			
			
		}
	});
	
	function materialColors(color){
		return {
			lighterColors:[
				{
					r: color.r,
